...
Kural needs an embedding provider to generate vectors. Set the `AI_GATEWAY_API_KEY` environment variable, or pass `--api-key` on each command.
...
| `domainKeywords`          | Top domain terms for path signal context                 |
| `dictionary`              | Codebase-specific term definitions for richer signatures |
| `audits.sensitivity`      | Standard deviations from mean to flag (default: 2.0)     |
| `audits.containmentFloor` | Absolute floor for containment detection (default: 0.9)  |
| `audits.minGroup`         | Minimum group size for statistical tests (default: 4)    |
