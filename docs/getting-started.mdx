...
Kural is a discipline layer for codebases — the structural counterpart to what TypeScript does for types. It answers _"where should this code live?"_ for humans and AI agents alike, scores how well your codebase is organized, and surfaces the drift that accumulates as code is added or moved. The pipeline runs on embedding-tier compute, so a full pass over a 50k-LOC codebase costs cents at hosted rates and zero with local models.

This page walks you from install to first audit. If you're here to give an AI agent structural discipline, jump to [AI Code Editors](/docs/ai-editors) after the snapshot step.
