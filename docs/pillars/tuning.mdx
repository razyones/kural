...
At **2.0** (default), findings are moderate. At **3.0**, only strong deviations surface. Below **1.5**, expect many findings. See [Configuration](/docs/configuration) for how to set it.
...
| `IQR_TO_MAD`               | 2     | `fence.ts`         | IQR &asymp; 2&middot;MAD for normal data. Converts IQR to MAD-equivalent scale; the robust spread is `max(MAD, IQR/2)`, which floors the spread when MAD shrinks faster than IQR.                                                                   |
