...
<Tabs items={["Neither signal", "Only causes", "Only calls", "Both"]}>
  <Tab value="Neither signal">
    ```
    signatureFacet = emb(signature)
    ```
  </Tab>
  <Tab value="Only causes">
    ```
    signatureFacet = 0.7 × emb(signature) + 0.3 × emb(causes)
    ```
  </Tab>
  <Tab value="Only calls">
    ```
    signatureFacet = 0.7 × emb(signature) + 0.3 × emb(calls)
    ```
  </Tab>
  <Tab value="Both">
    ```
