...
Annotations that declare structural realities the vector space can't capture. Reach for a param only after documentation fixes fail **and** you can state, in one sentence, which structural reality the param is declaring. Every param is a concession to reality, not a feature. The full specification lives in [`docs/codebase-realities/kural-params.md`](docs/codebase-realities/kural-params.md).
...
4. You can state, in one sentence, the structural reality the param is declaring. If you cannot, the right fix is a doc rewrite or a move, not a param.
